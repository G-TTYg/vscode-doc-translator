...
OpenAI-compatible provider 会根据 `maxContextTokens` 和 `maxOutputTokens` 先选择连续目标 translation units，再用滑动窗口在目标范围前后扩展参考上下文。每个请求返回扁平 JSON：`{ "translations": [{ "id": "...", "text": "..." }] }`。多个请求的结果合并后交给格式适配器重建最终文档。

分段预算规则：

- `maxContextTokens` 表示模型最大上下文窗口。
- `maxOutputTokens` 表示单次响应的最大输出 token，并作为 OpenAI-compatible API 的 `max_tokens` 发送。
