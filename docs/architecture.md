...
flowchart LR
  ...
  ProviderPort --> LLM["Shared structured LLM pipeline"]
  LLM --> OpenAI["OpenAI Responses adapter"]
  LLM --> Anthropic["Anthropic Messages adapter"]
  LLM --> Gemini["Gemini GenerateContent adapter"]
  LLM --> Compatible["OpenAI-compatible adapter"]
...
| `src/core/providers/` | Provider factory、OpenAI Responses/Anthropic/Gemini/OpenAI-compatible/DeepL/Google/Microsoft adapters、HTTP retry、传统 provider batching，以及共享的 AI 分段、目标语言检测、LLM JSON 恢复、有界补请求和 unit 级 source-preserving fallback。 |
...
Metadata 的 `target.relativePath` 始终从源文件目录计算；hidden-cache 模式下它会包含隐藏目录名。缓存命中必须同时匹配 source hash、目标语言、provider profile hash、输出模式和译文 hash。AI provider profile 包含 endpoint label、model、输入/输出预算和 harness version；profile 也包含 core version 与格式适配器版本，使旧实现生成的结果不会被新版质量规则继续复用。旧自动译文清理属于 core metadata store 责任，只有用户开启 `docTranslator.cache.deleteStaleAutoTranslations` 且旧译文 hash 仍匹配 metadata 时才会删除。
...
传统机器翻译 provider 使用 `segmented-units`。OpenAI Responses、Anthropic、Gemini 和 OpenAI-compatible provider 使用 `ordered-json-context`，并要求结构化 JSON 输出。
...
共享 `StructuredLlmProvider` 根据模型上下文和单次输出预算构造请求，各原生 provider 只负责认证、请求字段和响应文本提取：

1. `maxContextTokens` 表示模型最大上下文窗口。
2. `maxOutputTokens` 表示单次响应最大输出 token，并映射到各供应商对应字段。
...
共享 LLM 管线要求模型对 `translationUnitIds` 中的每个 id 都返回一条记录。原生 OpenAI、Anthropic 和 Gemini 适配器使用各自的 JSON Schema 结构化输出字段；OpenAI-compatible Chat Completions 使用 JSON mode。共享管线会校验未知、重复和缺失 id，兼容被 Markdown code fence 或说明文字包裹的 JSON、扁平 id 映射和单项对象。非法对象结构重试一次；缺失 id 定向补请求一次，仍缺失时用对应源文本补齐，确保格式适配器始终收到完整 id 集合。

质量门会检查有意义自然语言 unit 的译文是否仍与原文相同，并忽略大小写、空白和标点包装差异。若一个 chunk 中至少 50% 的候选 unit 仍是原文，只对这些 id 发起一次 `unchanged-repair` 请求；修复后仍相同的 unit 以源文本完成并记录 warning，不再中止整份文档。自动源语言检测确认整篇已主要使用目标文字时，provider 不发起 API 请求；显式源语言不使用该文档级捷径。

Protected token 使用文档内唯一名称。共享管线会把模型返回的原 token、Markdown 转义形式或对应原始代码/URL 统一还原为规范 token；真正缺失或重复时只对受影响 id 发起一次 `protected-token-repair`。若修复仍失败，为保证主要翻译流程完成，仅这些 unit 回退为带完整 token 的源文本，其余 unit 正常重建，并在结果 metadata 和 VS Code 成功提示中记录 warning。
...
- 旧自动译文清理默认关闭；开启后也只能删除同一源文件、目标语言、provider 和输出模式下未被编辑的旧自动产物。
...
| Provider adapters | Mock HTTP contract tests覆盖 URL、认证头、结构化输出字段和响应解析；真实 API 测试必须显式凭据启用。 |
| LLM structured output | ordered JSON context、missing/duplicate id、invalid/alternate JSON shape、already-target 文档、protected token 归一化/修复/局部回退、skip 语义和原文回显局部降级。 |
| AI large files | token 预算分段、目标 id 与参考窗口分离、供应商输出 token 字段传递、结果合并。 |
| Term locks | 用户配置术语在 provider round trip 中保持不翻译。 |
| VS Code 插件 | 命令注册、右键菜单贡献、设置 webview 和 SecretStorage smoke。 |

## 已知风险

- 所有外部 provider 已有 mock HTTP 合同测试，但仍需要用户凭据下的 live 验证。
- OpenAI-compatible provider 依赖服务兼容 Chat Completions JSON mode 和 `max_tokens` 语义；不同网关可能有差异。
...
- ADR：`docs/adr/2026-07-14-optional-stale-auto-translation-cleanup.md`
- ADR：`docs/adr/2026-07-20-native-llm-api-adapters.md`
- ADR：`docs/adr/2026-07-20-ai-source-echo-quality-gate.md`
- ADR：`docs/adr/2026-07-20-success-oriented-protected-token-recovery.md`
