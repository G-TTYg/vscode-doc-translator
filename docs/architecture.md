...
| `src/core/providers/` | Provider factory、OpenAI Responses/Anthropic/Gemini/OpenAI-compatible/DeepL/Google/Microsoft adapters、HTTP retry、传统 provider batching，以及共享的 AI 分段、LLM JSON 恢复、缺失 id 补请求、protected-token 恢复和原文回显质量门。 |
...
共享 LLM 管线要求模型对 `translationUnitIds` 中的每个 id 都返回一条记录。原生 OpenAI、Anthropic 和 Gemini 适配器使用各自的 JSON Schema 结构化输出字段；OpenAI-compatible Chat Completions 使用 JSON mode。共享管线会校验未知、重复和缺失 id，兼容被 Markdown code fence 或说明文字包裹的 JSON，并在第一次响应缺少 id 时，对缺失 unit 发起一次补请求。

质量门会检查有意义自然语言 unit 的译文是否仍与原文相同，并忽略大小写、空白和标点包装差异。若一个 chunk 中至少 50% 的候选 unit 仍是原文，只对这些 id 发起一次 `unchanged-repair` 请求；修复后比例仍达到 50% 时抛错，core 不进入重建和写文件阶段。已明确指定源语言与目标语言相同的请求，以及主要由中文、日文、韩文、西里尔、希腊或阿拉伯目标文字组成的原文，不进入该回显判定。
...
| LLM structured output | ordered JSON context、missing id、duplicate id、invalid JSON、protected token 归一化/修复/局部回退、skip 语义、原文回显修复和失败不落盘校验。 |
