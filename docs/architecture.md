...
| `src/core/providers/` | Provider factory、OpenAI Responses/Anthropic/Gemini/OpenAI-compatible/DeepL/Google/Microsoft adapters、HTTP retry、传统 provider batching，以及共享的 AI 分段、LLM JSON 恢复、缺失 id 补请求和原文回显质量门。 |
...
| LLM structured output | ordered JSON context、missing id、duplicate id、invalid JSON、protected token、skip 语义、原文回显修复和失败不落盘校验。 |
