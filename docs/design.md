...
构建一个 VS Code 插件和 CLI，用来快速、准确、可重复地翻译整份文档，并在支持的文件类型中尽量保留原文档结构与格式。
...
- 整份文档翻译应该是一键动作：VS Code 右键、命令面板或一条 CLI 命令。
- 永远不覆盖源文档。
- 译文文件名必须清楚标记为自动翻译，不能和原文档混淆。
- 保留格式是核心需求，不是翻译完之后再补救的附加项。
- 翻译服务必须可替换：Google、DeepL、Microsoft、OpenAI-compatible LLM 都通过适配器接入。
...
```bash
vscode-doc-translator translate ./guide.md --to zh-CN --provider deepl
```
...
--from auto
--to zh-CN
--provider deepl|google|microsoft|openai-compatible
--profile accurate|fast|cheap
--output same-dir
--output hidden-cache
--force
--dry-run
...
CLI 和 VS Code 插件必须共享核心实现，避免缓存规则、命名规则、格式解析和 provider 行为发生漂移。
...
| AI/LLM：OpenAI-compatible | 格式适配器生成按原文顺序排列的全文 JSON 上下文；provider 再按模型最大上下文构造滑动窗口。 | 请求中 `referenceDocument.units` 是参考上下文，`translationUnitIds` 是目标翻译 id；输出格式化扁平 JSON，例如 `{ "translations": [{ "id": "...", "text": "..." }] }`。每个目标 id 必须出现；如果某个 unit 不需要翻译，应返回 `{ "id": "...", "skip": true }`。Provider 会尝试恢复被包裹的 JSON，并对缺失 id 发起一次只包含缺失 unit 的补请求。 | 校验 id 完整性、重复、缺失和 protected token 后，由格式适配器重建译文文档。 |
...
- OpenAI-compatible LLM API，包括 OpenAI、Azure OpenAI-compatible endpoint、本地网关或其他兼容服务
...
- AI/LLM provider 必须支持 token 预算分段。当前主要设置是 `docTranslator.llm.maxContextTokens` 和 `docTranslator.llm.maxOutputTokens`：前者表示模型最大上下文窗口，后者表示单次响应请求的最大输出 token，并会作为 OpenAI-compatible API 的 `max_tokens` 发送。旧的 `docTranslator.llm.maxContextCharacters` 仅保留为兼容旧脚本的近似换算入口。
...
- OpenAI-compatible LLM provider 接收 ordered JSON context：按原文档顺序列出全部可翻译文本块、id、类型、上下文和 protected token。LLM 必须返回扁平 JSON 译文列表，并校验每个 id 恰好出现一次。
...
```json
{
  ...
  "docTranslator.defaultProvider": "openai-compatible",
...
当前实现默认 provider 是 `openai-compatible`。用户需要在可视化设置面板中配置 endpoint、model 和 API key，或切换到 DeepL、Google、Microsoft provider。
...
- CLI 优先从环境变量读取 secret，以后可支持 OS keychain。
...
- 翻译会把文档内容发送给用户选择的 provider。UI 和 CLI 必须明确提示这一点。
...
- 已完成：添加共享 core、VS Code extension、CLI 和测试设置。

### Phase 1：MVP

- 已完成：Markdown、plain text、MDX、HTML/XML 适配器第一版。
- 已完成：OpenAI-compatible provider、DeepL、Google、Microsoft provider 适配器。
