...
- AI/LLM provider 接收 ordered JSON context：按原文档顺序列出全部可翻译文本块、id、类型、上下文和文档内唯一的 protected token。LLM 必须返回扁平 JSON 译文列表，并校验每个 id 恰好出现一次。共享管线允许安全归一化 token 的 Markdown 转义形式或对应原值。
- 自动源语言检测确认整篇文档已主要使用目标文字时，AI provider 直接返回本地 source-preserving 结果，不发送无意义请求。
- AI 返回结构、缺失 id、原文回显或 protected token 异常时只执行有界定向修复；仍失败的 unit 保留原文并告警，完整 id 集合继续交给格式适配器，优先完成可用译文。
...
- 已部分完成：为 `accurate` profile 添加 LLM review/repair pass；当前已具备 protected-token 自动归一化、定向 repair、局部源文回退，以及大面积原文回显 repair；更广泛的术语与语义复核仍待实现。
