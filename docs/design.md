...
| AI/LLM：OpenAI-compatible | 格式适配器生成按原文顺序排列的全文 JSON 上下文；provider 再按模型最大上下文构造滑动窗口。 | 请求中 `referenceDocument.units` 是参考上下文，`translationUnitIds` 是目标翻译 id；输出格式化扁平 JSON，例如 `{ "translations": [{ "id": "...", "text": "..." }] }`。 | 校验 id 完整性、重复、缺失和 protected token 后，由格式适配器重建译文文档。 |
...
```json
{
  ...
  "docTranslator.defaultProvider": "fake",
...
当前实现中 `fake` 是本地开发和自动化测试默认 provider，避免首次运行必须配置外部 API。发布前需要把默认 provider 或首次运行向导调整为真实翻译服务选择。
...
- 已完成：OpenAI-compatible provider、本地 fake provider、DeepL、Google、Microsoft provider 适配器。
